...
# ---- Chapters -----------------------------------------------------------------
chapters <- data.frame(
  id    = c("intro","storms","mortality","merge","model"),
  title = c(
    "Introduction",
  ...
  text  = c(
    "Scroll or click chapters. Map will pan/zoom and toggle layers.",
    "Focus on Florida and recent hurricane tracks/landfalls.",
  ...
  lng   = c(-83.5, -83.5, -82.3, -82.3, -81.6),
  lat   = c(28.0,   28.0,   27.0,   27.8,   27.2),
  zoom  = c(6,      6,      7,      7,      7),
  layer = c(NA, "storms", "mort", "merge", "model"),
  stringsAsFactors = FALSE
)

# ---- Dummy data (replace with yours) ------------------------------------------
storms_df <- data.frame(
  name=c("Ian (2022)","Idalia (2023)","Debby (2024)"),
  lng=c(-81.9,-83.0,-82.0), lat=c(26.9,29.0,28.3)
)
mort_df <- data.frame(
  name=c("County mortality A","County mortality B","County mortality C"),
  lng=c(-82.44,-81.65,-80.20), lat=c(28.04,27.77,26.64)
)
merge_df <- data.frame(
...
server <- function(input, output, session){
  ...
  output$map <- renderLeaflet({
    leaflet(options = leafletOptions(minZoom = 4)) %>%
      ...
      addCircleMarkers(data = storms_df, lng = ~lng, lat = ~lat, label = ~name, radius = 8, group = "storms") %>%
      addCircleMarkers(data = mort_df,   lng = ~lng, lat = ~lat, label = ~name, radius = 7, group = "mort") %>%
